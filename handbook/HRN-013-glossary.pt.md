...
title: "Glossário"
locale: "pt"
source: "https://santismm.com/pt/handbook/glossary"
---
# Glossário

## Resumo executivo

Este glossário é a referência canónica dos termos usados em todo o manual de Engenharia de Harness e no resto da Santismm Knowledge Platform. As definições são concisas, extraíveis por máquina e pensadas para serem citadas diretamente. Quando um termo tem capítulo próprio, a entrada remete para ele.

## Definição

O que se segue são as definições canónicas dos termos usados ao longo deste corpus. Os termos estão agrupados por legibilidade; dentro de cada grupo vão aproximadamente do fundacional ao especializado.
...
- **Sistema agêntico:** um sistema de software cujo comportamento é conduzido por um ou vários agentes, incluindo todo o andaime circundante necessário para os tornar fiáveis.
- **Harness:** o andaime de engenharia em torno de um modelo — memória, ferramentas, planeamento, orquestração, observabilidade, avaliação, governação e segurança — que converte um modelo em bruto num sistema agêntico fiável.
- **Engenharia de Harness:** a disciplina responsável por construir sistemas agênticos fiáveis para ambientes empresariais, desenhando e operando o harness.
...
- **Gradiente de autonomia:** o espectro de modos de controlo, de totalmente autónomo a humano no ciclo, atribuído por política a cada classe de ação.
...
- **Lista de permitidos (*allowlist*):** um conjunto explícito de elementos autorizados (ferramentas, destinos de saída); o valor por omissão seguro nas decisões sensíveis à segurança.
...
- **RAG (geração aumentada por recuperação):** fornecer ao modelo conteúdo relevante recuperado em tempo de inferência, de modo que a sua saída fique ancorada num corpus e não apenas na memória paramétrica.
...
- **Replaneamento:** rever um plano em resposta a uma falha, a informação nova ou a uma mudança de restrições.
...
- **Agente supervisor (orquestrador):** um agente que planeia e delega em agentes trabalhadores (ver PAT-002).
...
- **Passagem (*handoff*):** transferência de controlo e contexto de um agente para outro.
- **Máquina de estados:** um grafo explícito de estados e transições governadas que controla a execução.
- **Execução duradoura:** semântica de fluxo em que o progresso é persistido em pontos de controlo e é retomável perante as falhas.
...
- **Governação:** a camada de execução do harness que aplica política, aprovações e guarda-corpos (ver HRN-008).
- **Política como código:** regras de governação expressas num formato declarativo, versionado e testável.
- **PEP / PDP:** ponto de aplicação de política (interceta as ações) e ponto de decisão de política (avalia a política).
...
- **DLP (prevenção de fuga de dados):** controlos que detetam e bloqueiam dados sensíveis nas cargas de saída.

### Observabilidade e avaliação

- **Observabilidade:** a camada do harness que torna inspecionável o comportamento do agente através de traços, registos e métricas (ver HRN-006).
- **Traço:** o registo de ponta a ponta de uma única execução do agente.
...
- **Avaliação (*eval*):** a medição sistemática da qualidade, da segurança e da fiabilidade do agente (ver HRN-007).
...
- **Esquema de ferramenta:** a declaração tipada do nome, dos argumentos e da descrição de uma ferramenta, que o modelo usa para a invocar.
- **llms.txt:** uma convenção proposta para um ficheiro Markdown ao nível do sítio que expõe um índice de conteúdo curado e apto para agentes.
