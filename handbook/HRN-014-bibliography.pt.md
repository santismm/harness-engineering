...
title: "Bibliografia"
locale: "pt"
source: "https://santismm.com/pt/handbook/bibliography"
---
# Bibliografia
...
- Wang, X. et al. **«Plan-and-Solve Prompting».** ACL. Decomposição de planear e depois executar.

### 2. Orquestração, multiagente e execução duradoura

- Anthropic. **«Building Effective Agents».** Orientação de engenharia sobre fluxos face a agentes e sobre o desenho com agente único primeiro.
...
- Lewis, P. et al. **«Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks».** NeurIPS. O artigo canónico de RAG.
...
- Zheng, L. et al. **«Judging LLM-as-a-Judge with MT-Bench and Chatbot Arena».** A metodologia do LLM como juiz e os seus enviesamentos.
...
- Willison, S. **«Prompt injection»** e **«The lethal trifecta for AI agents»** (ensaios de blogue). A formulação mais clara do problema arquitetónico de injeção e exfiltração.
...
- NIST. **SP 800-53** (controlos de segurança e privacidade; privilégio mínimo, identidade), adaptado a sistemas de IA.

### 6. Governação, risco e normas regulatórias
...
- Ver também GOV-001 (quadro de governação de IA empresarial) e GOV-005 (lista de verificação de controlos de governação de agentes) neste corpus.
...
- Proposta **llms.txt** — uma convenção ao nível do sítio para indexar conteúdo de forma apta para agentes.
...
R: GOV-001 leva à prática as fontes de governação e regulação das secções 5 e 6 dentro de um quadro empresarial; esta bibliografia é a lista de leituras que está por baixo.

**P: Por onde deve começar quem chega de novo?**
R: Pela secção 1 (ReAct, Reflexion) para perceber como funcionam os agentes, pela secção 2 («Building Effective Agents») para saber como os construir de forma fiável, e pelas secções 5 e 6 para segurança e governação.
